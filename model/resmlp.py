...
class FCN_Block(nn.Module):
    ...
    def __init__(self, in_feats: int, out_feats: int, bnorm_in_feats: int = None,
                 dropout: float = 0.1, add_IC: bool = True, act_layer: nn.Module = nn.ReLU):
        super().__init__()
        ...
        if self.add_IC:
            self.norm_layer = nn.BatchNorm1d(
                bnorm_in_feats if bnorm_in_feats else in_feats)
...
class ResMLP(BaseModel):
    ...
    def __init__(self, dropout: float, num_residuals_per_block: int, num_blocks: int, num_classes: int,
        ...
        if reduce_in_feats is not None:
            ...
            blocks.append(FCN_Block(
                in_feats=num_initial_feats, out_feats=reduce_in_feats, dropout=dropout, add_IC=add_IC))
        else:
    ...
    def _create_block(self, in_feats: int, dropout: float,
        ...
        block.append(FCN_Block(
            in_feats=in_feats, out_feats=in_feats // 2, dropout=dropout, add_IC=add_IC))  # add downsampling
        return block
...
class ResMLPFeatBottleneck(BaseModel):
    """
    MLP with optinally batch norm, dropout, and residual connections. I got
    inspiration from the original ResNet paper and https://arxiv.org/pdf/1905.05928.pdf.
    ...
    def __init__(self, dropout: float, num_residuals_per_block: int, num_blocks: int, num_classes: int,
                 num_frames: int, num_initial_feats: int, reduce_in_feats: int, add_residual: bool = True, add_IC: bool = True):
        super().__init__()
        ...
        if reduce_in_feats is not None:
            # add downsampling
            blocks.append(FCN_Block(
                in_feats=num_initial_feats, out_feats=reduce_in_feats, bnorm_in_feats=num_frames, dropout=dropout, add_IC=add_IC))
        else:
            reduce_in_feats = num_initial_feats

        blocks.append(ViewBlock([num_frames * reduce_in_feats]))
        reduce_in_feats = num_frames * reduce_in_feats
        # print(reduce_in_feats)
        for i in range(num_blocks):
            blocks.extend(self._create_block(
                reduce_in_feats, dropout, num_residuals_per_block, add_residual, add_IC, i))
            reduce_in_feats //= 2
